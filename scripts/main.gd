...
const MAX_WORD_LENGTH: int = 6 ## changed from 5 to 6
...
const hint_speed: float = 2.0 ## new addition
const error_scale: float = 1.2
const hint_scale: float = 1.2 ## new addition
...
@onready var hint_box: Label = $HintBox ## new addition
...
var word_list_all: Array[String]## name change
var word_list: Array[String] ## new addition
var hint_list: Array[String] ## new addition

var word: String
var max_guesses: int
var arrow_tween: Tween

var show: bool = true ## new additon in reference to the hint toggle
var can_type: bool = true
## erased due to the removal of the info label and more button
...
func initialize_round():
	...
	error_box.hide()
	hint_box.hide() ## hint box reset
	max_guesses = guesses.get_child_count()
	rounds_text.text = str(round_count)
	current_block = 0
	current_row = 0
	can_type = true
	show = false ## fixed value
	## erased due to the removal of the info label and more button

# pick new word
func pick_new_word():
	word = word_list.pick_random() ## same name, differnt txt file, contains only a select few words
	#print(word)
...
func show_hint(message: String):
	message = hint_list[word_list.find(word)]
	hint_box.text = message
	hint_box.pivot_offset.x = hint_box.size.x / 2.0
	var hint_tween = get_tree().create_tween().set_ease(Tween.EASE_OUT
		).set_trans(Tween.TRANS_CUBIC)
	hint_tween.tween_property(hint_box, "scale", Vector2.ONE, 1/hint_speed
		).from(Vector2(hint_scale, hint_scale))
...
func confirm_answer():
	...
	if current_block < MAX_WORD_LENGTH:
		...
		return
	elif not get_guess() in word_list_all:
...
func _ready():
	...
	## changed file and array names
	if FileAccess.file_exists("res://assets/word_list_all.txt"):
		var file = FileAccess.open("res://assets/word_list_all.txt", FileAccess.READ)
		while not file.eof_reached():
			var line = file.get_line()
			if not line in word_list_all:
				word_list_all.append(line.to_lower())
		file.close()
##Azerdle original code\

	## a txt file that contains definitions for words in word_list.txt
	if FileAccess.file_exists("res://assets/hint_list.txt"):
		var file = FileAccess.open("res://assets/hint_list.txt", FileAccess.READ)
		while not file.eof_reached():
			var line = file.get_line()
			if not line in word_list_all:
				hint_list.append(line.to_lower())
		file.close()
		
	## a txt file that contains a smaller amount of words than the
	## word_list_all.txt making geussing easier
...
## removed the signal for more button
...
## added the signal for hint toggle button
func _on_hints_toggled(toggled_on: bool) -> void:
	confirm_sound.play()
	## makes sure that the hint button has to be pressed only once
	if show != toggled_on:
		show = toggled_on
		if show == true:
			hint_box.show()
